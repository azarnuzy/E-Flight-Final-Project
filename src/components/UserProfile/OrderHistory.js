...
export default function OrderHistory() {
  ...
  const { history, isLoading, hasError } = useSelector((state) => state.orderHistory)
        ...
        {history && history.map((res, index) => (
          <div className='bg-slate-200 p-3 mt-4 rounded-md flex flex-col gap-3'>
            ...
            <div className='flex items-center gap-3 font-semibold'>
              <h1 className=''>{res.originAirport}</h1> <HiArrowRight /> <h1>{res.destinationAirport}</h1>
            </div>
            <div className='flex flex-wrap gap-5 gap-y-1 '>
              <div className='flex items-center'>
                <p>Sekali Jalan</p><VscDebugStackframeDot /> <p>{res.totalPassenger} dewasa</p>
              </div>
            ...
            <div className='flex justify-between mt-5'>
              <span className=' text-slate-400 px-3 p-1 rounded-lg text-sm'>{res.bookingStatus}</span>
              <Link className='text-secondary font-medium hover:text-primary'> See Detail </Link>
            </div>
          </div>
        ))}
      </div >
